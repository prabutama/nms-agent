...
func parseGenericMQTTConfig(cfg map[string]any) (genericMQTTConfig, error) {
	c := genericMQTTConfig{
		QoS:            1,
		Retain:         false,
		AutoReconnect:  true,
	...
	// qos can be an int (YAML) or string.
	...
	// Accept tcp://, ssl://, tls://. If user passes host:port, assume tcp.
...
// genericMQTTClient is a minimal wrapper for testability.
...
func NewGenericMQTTAdapter(cfg map[string]any) (*GenericMQTTAdapter, error) {
	...
	if c.StrictQueue {
		// Strict mode: fail-fast so SQLite queue reflects broker outages.
		// Disable reconnect buffering behavior in the MQTT client.
...
func (a *GenericMQTTAdapter) ensureConnected(ctx context.Context) error {
	...
	// Connect is async; wait with timeout.
...
func (a *GenericMQTTAdapter) SendBatch(ctx context.Context, batch []models.Telemetry) error {
	...
	for _, t := range batch {
		out := t
		out.TS = t.TS.In(getOutputLocation())
...
func (a *GenericMQTTAdapter) Close() error {
	...
	// Quiesce 250ms to allow pending work.
