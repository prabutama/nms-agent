...
func (m tuiModel) deviceDetailView(width int) string {
	...
	if len(da) > 0 {
		...
		for _, a := range da {
			status := a.Status
			line := fmt.Sprintf("    - %s=%s %s  %s", a.Metric, a.Value, status, a.Updated.In(loc).Format("15:04:05"))
			if width > 0 {
				line = truncatePlain(line, width)
			}
			if status == "critical" {
				line = strings.Replace(line, status, styleCrit.Render("CRITICAL"), 1)
			} else {
				line = strings.Replace(line, status, styleWarn.Render("WARNING"), 1)
			}
			b += line + "\n"
		}
	}
	...
	if len(ifaces) > 0 {
		...
		for _, s := range ifaces {
			...
			line := fmt.Sprintf("    %-11s %-11s %-11s %s", ifName, formatBits(s.RxBps), formatBits(s.TxBps), utilStr)
			if width > 0 {
				line = truncatePlain(line, width)
			}
			b += line + "\n"
