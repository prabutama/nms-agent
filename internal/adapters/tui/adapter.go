...
func (a *TUIAdapter) SetObserver(hub base.AdapterObserver) {
	a.obs = hub
}
...
func parseConfig(cfg map[string]any) tuiConfig {
	c := tuiConfig{
...
func NewAdapter(cfg map[string]any) (*TUIAdapter, error) {
	c := parseConfig(cfg)
...
func newProgramForTest(m tea.Model, out io.Writer) *tea.Program {
	if out == nil {
		out = io.Discard
	}
