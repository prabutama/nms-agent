...
| `internal/adapters/thingsboard_mqtt_adapter.go` | ThingsBoard MQTT adapter Phase 8: publish canonical telemetry ke ThingsBoard Gateway API (`v1/gateway/telemetry`) dengan metadata tags/threshold, plus key flattened untuk metric indexed memakai `ifName` tersanitasi (lowercase, separator `-`) atau fallback `idx<ifIndex>`. |
...
| `internal/adapters/thingsboard_mqtt_adapter_test.go` | Unit test ThingsBoard MQTT adapter: validasi config + payload shape + flattened indexed keys (`ifName`/`ifIndex`) termasuk sanitasi nama interface + publish error/strict mode tanpa broker real.                      |
