...
| `internal/adapters/thingsboard_mqtt_adapter.go` | ThingsBoard MQTT adapter Phase 8: publish canonical telemetry ke ThingsBoard Gateway API (`v1/gateway/telemetry`); khusus `snmp.if.*` dan `snmp.host.storage.*` bertag `ifIndex`, adapter hanya mengirim key flattened (interface pakai `ifName` tersanitasi atau fallback `idx<ifIndex>`, storage pakai `idx<ifIndex>`), sementara metric lain tetap generic. |
