...
| `internal/integrations/thingsboard/alarm_manager.go` | Orchestrator alarm lifecycle berbasis threshold telemetry: mapping metric→alarm type, set propagation flags, create/update saat critical/warning, clear saat normal, dan cache alarm per device+metric. |
