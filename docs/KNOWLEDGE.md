...
| `internal/adapters/thingsboard_mqtt_adapter.go` | ThingsBoard MQTT adapter Phase 8: publish canonical telemetry ke ThingsBoard Gateway API (`v1/gateway/telemetry`); khusus `snmp.if.*` bertag `ifIndex`, adapter hanya mengirim key flattened memakai `ifName` tersanitasi (lowercase, separator `-`) atau fallback `idx<ifIndex>`, sementara metric lain tetap generic. |
...
| `internal/adapters/thingsboard_mqtt_adapter_test.go` | Unit test ThingsBoard MQTT adapter: validasi config + payload shape + flattened `snmp.if.*` keys (`ifName`/`ifIndex`) termasuk sanitasi nama interface, guard non-interface indexed metrics tetap generic, serta publish error/strict mode tanpa broker real.                      |
