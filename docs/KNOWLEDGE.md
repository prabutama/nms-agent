...
| `cmd/nms-agent/main.go`        | Entrypoint agent service. Memilih collector runtime via `--collector-mode`, adapter via factory (`tui`/`generic_mqtt`/`thingsboard_mqtt`), menjalankan pipeline periodik, dan auto-reload `devices.d` via watcher. Discovery tidak lagi berjalan di loop daemon. |
...
| `internal/config/types.go`     | Definisi struct config (root agent.yml, device entry, thresholds/adapters, dan tipe discovery untuk CLI runtime) + `ResolvePath()` untuk relative path + env expansion, `Delivery.WithDefaults()`. `agent.yml` tidak lagi memakai `discovery.interval`. |
...
| `internal/config/validate.go`  | Validasi dasar konfigurasi daemon (field wajib, duplikasi device id, sanity-check YAML) tanpa mewajibkan discovery config, + `ValidateThresholdRules()` untuk isolated rule validation.    |
...
| `internal/adapters/thingsboard_mqtt/adapter.go` | ThingsBoard MQTT adapter: auto-provision device tokens via `/api/v1/provision`, cache token in SQLite table `thingsboard_device_tokens`, publish per-device payloads to `v1/devices/me/telemetry`, project route string records to attributes, and run management side-effects (relation reconciler + topology publisher + alarm manager) with stderr diagnostic logging. |
| `internal/adapters/thingsboard_mqtt/route_test.go` | Unit test route inventory projection: route summary stays telemetry while route string detail is published as attributes. |
| `internal/integrations/thingsboard/models.go` | Model integration ThingsBoard untuk site context, relation, alarm propagation flags, device info, dan topology snapshot site-local. |
...
| `internal/integrations/thingsboard/alarm_manager.go` | Orchestrator alarm lifecycle berbasis threshold telemetry: mapping metric→alarm type, set propagation fields, create/update saat critical/warning, clear saat normal, dan cache alarm per device+metric. |
...
| `internal/processors/preprocess_threshold_processor.go` | Preprocess metric ms/pct dengan rounding 2 desimal, plus derivation SNMP memory dan hrStorage (`used_pct`, bytes) sebelum threshold evaluation dan alarm flow. |
...
| `cmd/nms-agentctl/discover.go` | CLI discovery manual: `status`, `preview`, dan `run` sebagai satu-satunya entrypoint discovery. `preview/run` wajib `--subnet`, dapat override provider/SNMP settings via flags, dan auto-detect interface bila `--interface` tidak diberikan.                                                     |
